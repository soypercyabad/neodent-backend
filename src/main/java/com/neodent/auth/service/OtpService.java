...
public class OtpService {
    ...
    public Usuario verificarLoginOtp(
        Long challengeId,
        String codigo
    ) {

        CodigoVerificacion otp = repository
            .findByIdAndTipo(challengeId, TIPO_LOGIN)
            .orElseThrow(() ->
    ...
    @Transactional
    public OtpGenerado generarEmailVerification(
        Usuario usuario
    ) {
    ...
    public Usuario verificarEmailOtp(
        Long challengeId,
        String codigo
    ) {
    ...
    @Transactional
    public OtpGenerado reenviarCodigo(
        Long challengeId
    ) {
        ...
        short reenvios =
            anterior.getResendCount() == null
                ? 0
                : anterior.getResendCount();

        if (reenvios >= MAX_REENVIOS) {
            throw new UnauthorizedException(
                "Se alcanzó el máximo de reenvíos permitidos"
            );
        }
        ...
        if (
            anterior.getLastResendAt() != null &&
            ahora.isBefore(
                anterior.getLastResendAt()
                    .plusSeconds(
        ...
        OtpGenerado nuevo =
            switch (anterior.getTipo()) {
                ...
                default ->
                    throw new UnauthorizedException(
                        "Tipo de verificación no permitido"
                    );
            };
    ...
    public OtpGenerado reiniciarEmailVerification(
        ...
        List<CodigoVerificacion> anteriores =
            repository
                .findAllByUsuarioIdAndTipoAndUsadoFalse(
                    usuario.getId(),
                    TIPO_EMAIL
                );

        anteriores.forEach(
            otp -> otp.setUsado(true)
        );
